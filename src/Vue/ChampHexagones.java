...
public class ChampHexagones extends JPanel implements MouseListener {
    private int rows;
    private int columns;
    private int hexSize;
    private Color[][] hexColors;

    public ChampHexagones(int rows, int columns, int hexSize) {
        this.rows = rows;
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (int row = 0; row < rows; row++) {
            for (int col = 0; col < columns; col++) {
                ...
                Color color = hexColors[row][col] != null ? hexColors[row][col] : Color.RED;
